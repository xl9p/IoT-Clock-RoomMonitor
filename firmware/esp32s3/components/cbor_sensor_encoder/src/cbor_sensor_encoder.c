...
encode_sensor_payload(const sensor_payload_t *payload, CborEncoder *sensors_array) {
  ...
  cbor_encoder_create_array(&map, &fields_array, CborIndefiniteLength);

  for (size_t i = 0; i < payload->field_count; i++) {
    const sensor_field_t *field = &payload->fields[i];

    CborEncoder field_map;
    cbor_encoder_create_map(&fields_array, &field_map, 2);

    cbor_encode_text_stringz(&field_map, "name");
    cbor_encode_text_stringz(&field_map, field->name);

    cbor_encode_text_stringz(&field_map, "value");

    switch (field->type) {
    case SENSOR_FIELD_DATATYPE_FLOAT:
      cbor_encode_float(&field_map, field->value.f);
      break;
    case SENSOR_FIELD_DATATYPE_INT:
      cbor_encode_int(&field_map, field->value.i);
      break;
    case SENSOR_FIELD_DATATYPE_LONG_INT:
      cbor_encode_int(&field_map, field->value.i);
      break;
    case SENSOR_FIELD_DATATYPE_UINT:
      cbor_encode_uint(&field_map, field->value.u);
      break;
    case SENSOR_FIELD_DATATYPE_LONG_UINT:
      cbor_encode_uint(&field_map, field->value.u);
      break;
    case SENSOR_FIELD_DATATYPE_BOOL:
      cbor_encode_boolean(&field_map, field->value.b);
      break;
    default:
      cbor_encode_null(&field_map);
      break;
    }

    cbor_encoder_close_container(&fields_array, &field_map);
  }

  cbor_encoder_close_container(&map, &fields_array);
